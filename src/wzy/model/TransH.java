...
public class TransH extends EmbeddingModel{
	...
	protected void CalculateGradient(int[] triplet)
	{
		...
		if(truesimi+margin-falsesimi>0)
		{
			...
			{
				wsum+=relationweight[triplet[1]][i];
				whsum+=relationweight[triplet[1]][i]*entityEmbedding[triplet[0]][i];
				wtsum+=relationweight[triplet[1]][i]*entityEmbedding[triplet[2]][i];
			}
			for(int i=0;i<truevector.length;i++)
			{
				if(truevector[i]>0)
				{
					entityGradient[triplet[0]][i]+=1-relationweight[triplet[1]][i]*wsum;
					relationGradient[triplet[1]][i]+=1;
					entityGradient[triplet[2]][i]+=relationweight[triplet[1]][i]*wsum-1;
					relationweightGradient[triplet[1]][i]+=wtsum-whsum
							+(entityEmbedding[triplet[2]][i]-entityEmbedding[triplet[0]][i])*wsum;
				}
				else
				{
					entityGradient[triplet[0]][i]-=1-relationweight[triplet[1]][i]*wsum;
			...
			{
				wsum+=relationweight[ftriplet[1]][i];
				whsum+=relationweight[ftriplet[1]][i]*entityEmbedding[ftriplet[0]][i];
				wtsum+=relationweight[ftriplet[1]][i]*entityEmbedding[ftriplet[2]][i];
			}
			for(int i=0;i<falsevector.length;i++)
			{
				if(falsevector[i]<0)
				{
					entityGradient[ftriplet[0]][i]+=1-relationweight[ftriplet[1]][i]*wsum;
					relationGradient[ftriplet[1]][i]+=1;
					entityGradient[ftriplet[2]][i]+=relationweight[ftriplet[1]][i]*wsum-1;
					relationweightGradient[ftriplet[1]][i]+=wtsum-whsum
							+(entityEmbedding[ftriplet[2]][i]-entityEmbedding[ftriplet[0]][i])*wsum;
				}
				else
				{
					entityGradient[ftriplet[0]][i]-=1-relationweight[ftriplet[1]][i]*wsum;
	...
	private double[] CalculateTripletVector(int[] triplet)
	{
		...
		{
			hs+=entityEmbedding[triplet[0]][i]*relationweight[triplet[1]][i];
			ts+=entityEmbedding[triplet[2]][i]*relationweight[triplet[1]][i];
		}
		
		
		for(int i=0;i<entity_dim;i++)
		{
			htran[i]=entityEmbedding[triplet[0]][i]-hs*relationweight[triplet[1]][i];
			ttran[i]=entityEmbedding[triplet[2]][i]-ts*relationweight[triplet[1]][i];
		}
		for(int i=0;i<entity_dim;i++)
		{
...
/*	private void RegularRelationWeight()
	...
	private void RegularRelationWeight()
	{
		for(int i=0;i<relationweight.length;i++)
		{
			double x=MatrixTool.VectorNorm2(relationweight[i]);
			if(x>0.5)
			for(int j=0;j<relationweight[i].length;j++)
			{
				relationweight[i][j]/=x;
